...
2. Re-running `./specc examples/BubbleSort` on a second pass should log at least one similar prior with similarity ≥ 0.85 — the outer `LoopNode` should match prior Loop nodes.
...
`./specc examples/BubbleSort` completes through `SemanticNormalizationPass` without error. The artifacts directory contains `02-semantic-graph.json`, `02c-semantic-graph.svg`, and `03-embeddings.json`. The repository `index.json` contains a BubbleSort entry. A second run logs at least one prior with similarity ≥ 0.85.
