...
using IrOp = Specc.Graph.OpCode;   // alias to avoid clash with System.Reflection.Emit.OpCode
        ...
        foreach (var instr in context.StackIr)
        {
            switch (instr.Op)
            {
                ...
                case IrOp.RandInt:
                    ...
                    break;
                case IrOp.Intrinsic:
                    var descriptor = Specc.Graph.IntrinsicLibrary.Get(instr.Operand!);
                    foreach (var step in descriptor.Steps)
                    {
                        switch (step)
                        {
                            case Specc.Graph.StaticCall sc:
                                il.EmitCall(OpCodes.Call, sc.Method, null);
                                break;
                            case Specc.Graph.VirtualCall vc:
                                il.EmitCall(OpCodes.Callvirt, vc.Method, null);
                                break;
                            case Specc.Graph.StaticGet sg:
